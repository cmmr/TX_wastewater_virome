...
harris_pos_table=sprintf(
  "%s/data/harris_county_sars_Positivity_Rate_230309.csv", 
  find_rstudio_root_file())
...
ggplot() + 
  ...
  geom_text(aes(x= as.Date('2022-11-01'), y = 22, label = "Summer Wave\n"), angle = 90) +
  geom_text(aes(x= as.Date('2022-11-01'), y = 22, label = "\nWinter Wave"), angle = 90) +
  scale_color_manual(name='Data Type',
                     breaks=c('Virome Seq', 'Average Positivity %'),
                     values=c('Virome Seq'='#4B4E55', 'Average Positivity %'='#F4A65E')) +
...
merge_sars_dt %>%
  ...
  labs(y = "Wastewater Sequencing \nAbundance (RPKMF)", 
       x = "Avg. Daily Average Positivity % (PCR test)") +
  stat_cor(label.x = 3) +
  theme_bw()
