...
metadata_table=sprintf(
  ...
  find_rstudio_root_file())
harris_case_table=sprintf(
  "%s/data/HTX_HARRIS_COVID_Cases_Timeline_230309.csv", 
  find_rstudio_root_file())
...
```{r}
epi_data <- fread(harris_case_table, sep = ",", header = T)


epi_data$Date <- as.Date(epi_data$Date_Label, , "%m-%d-%Y")


epi_data <- epi_data %>%
  # keep only city of Houston data, 
  filter(Source == "HOU") %>%
  # calculate "diff"  as days between adjacent sampling points using difftime function
  mutate(diff = c(difftime(tail(Date, -1), head(Date, -1)),0))

# parse diff column to numeric
epi_data$diff <- epi_data$diff %>% gsub(" days", "", .) %>% gsub("-", "", .)
epi_data$diff <- as.numeric(epi_data$diff)

# calculate new cases per day (Rate)
epi_data$Rate <- epi_data$New_Cases / epi_data$diff

# remove Inf
epi_data <- epi_data %>% filter(!grepl("Inf", Rate))
...
ggplot() + 
  geom_point(data = SARS_HTX_week_dt, 
            aes(x=Week, y=1200*(RPKMF), color="Virome Seq"), 
            alpha = 0.2, size = 1) +
  ...
  geom_text(aes(x= as.Date('2022-11-01'), y = 500, label = "Summer Wave\n"), angle = 90) +
  geom_text(aes(x= as.Date('2022-11-01'), y = 500, label = "\nWinter Wave"), angle = 90) +
  ...
  scale_y_continuous(name = "Avg. Daily Case Rate (PCR test)",
    sec.axis = sec_axis( trans=~./1200, name="Wastewater Sequencing \nAbundance (RPKMF)", label=scales::comma)
  ) +
  scale_x_date(date_labels="%Y-%b",date_breaks  ="1 month")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 

ggsave(sprintf("%s/charts/SARS-CoV2_HTX_virome_vs_cases1.pdf", find_rstudio_root_file()), width = 5, height = 4)
```
...
range_epi_dt_week <- rename(range_epi_dt_week, value = Rate)
...
ggsave(sprintf("%s/charts/SARS-CoV2_HTX_virome_vs_cases_wave_scatter1.pdf", find_rstudio_root_file()), width = 4, height = 3)
