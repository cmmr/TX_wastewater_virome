---
title: "compare all high-coverage samples for non-synonymous mutations in astrovirus MLB1 vs ref from H7HGKF"
...
variant_table=sprintf(
  "%s/data/all_astrovirus_MLB1.REF.H7HGKF.ivar.nonsyn_variants.tsv", 
...
mut_describe_dt <- nonsyn_info_dt %>%
  ...
  mutate(
    mutation_description = 
      paste(genome_position, gene_name, 
...
ggsave(sprintf("%s/charts/all_astro_MLB1_tSNE_site_city.pdf", find_rstudio_root_file()), width = 5.5, height = 4)
...
p %<+% meta_dt + 
  #layout_dendrogram() + 
  geom_tippoint(aes(fill=factor(City), x=x-0.01), 
                size=2, shape=21, color='black', alpha = 0.5) + 
  geom_tippoint(aes(fill=factor(Site), x=x+0.01), 
                size=2, shape=21, color='black', alpha = 0.5) +
...
pal <- wes_palette("FantasticFox1", 20, type = "continuous")
...
pm_labs
#ggsave("all_astro_MLB1_hclust_ggtree_heatmap2.pdf", width = 9, height = 8)
ggsave(pm_labs, file = sprintf("%s/charts/astrovirus_MLB1_ggtree_heatmap2.pdf", find_rstudio_root_file()), width = 8, height = 6)
