---
title: "compare all high-coverage samples for non-synonymous mutations in JC Polyomavirus vs GenBank Ref J02226.1"
...
variant_table=sprintf(
  "%s/data/all_JCPyV_genbank_J02226.1.nonsyn_variants2.tsv", 
  find_rstudio_root_file())
metadata_table=sprintf(
  "%s/data/wastewater_sample_metadata_table1.tsv", 
  find_rstudio_root_file())
CDS_table=sprintf(
  "%s/data/J02226.1_CDS_table1.tsv", 
  find_rstudio_root_file())
...
emb <- Rtsne::Rtsne(tephi_prcomp1$x[,1:10], perplexity = 10, exaggeration=4)
...
ggsave(sprintf("%s/charts/all_JCPyV_tSNE_site_city.pdf", find_rstudio_root_file()), width = 5.5, height = 4)
...
pal <- wes_palette("IsleofDogs1", 7, type = "continuous")
labels <- little_gene %>%
...
ggsave(pm_labs, file = sprintf("%s/charts/JCPyV_ggtree_heatmap2.pdf", find_rstudio_root_file()), width = 8, height = 6.5)
