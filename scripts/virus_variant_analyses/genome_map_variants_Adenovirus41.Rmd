---
title: "genome maps with non-synonmous mutations, Adenovirus 41"
...
gene_table$molecule <- "Adenovirus 41"
...
ordered_date_mut <- merge(mut_describe_dt, meta_dt, by = "sample_ID") %>%
  group_by(gene_tag, gene_codon_num) %>%
  filter(n() > 20 & n() < 145) %>%
  arrange(gene_tag, gene_codon_num, Date) %>%
...
palette for genes
```{r}
n_genes <- length(unique(gene_table$gene_tag))


pal_gene <- wes_palette("Darjeeling2", n_genes, type = "continuous")
```
...
gene_table <- gene_table %>%
  mutate(orientation = case_when(gene_orient == "+" ~ TRUE,
                                 gene_orient == "-" ~ FALSE))
...
genep <- ggplot() +
  geom_gene_arrow(data=gene_table, aes(xmin = gene_start, xmax = gene_end, 
             y = molecule, fill = gene_tag, forward = orientation),
  ...
  geom_gene_label(data=gene_table, aes(xmin = gene_start, xmax = gene_end,
                                       y = molecule, fill = gene_tag,
                                       label = gene_tag), align = "left") +
...
codon_list <- c("A", "C", "D", "E", "F", "G", "H", "I", "K", "L", "M", "N", 
                "O", "P", "Q", "R", "S", "T", "U", "V", "W", "Y", "STOP")
n_AA <- length(codon_list)

pal1 <- wes_palette("Moonrise3", n_AA, type = "continuous")

names(pal1) <- codon_list
```
...
ggsave(w_legend_combp, file = sprintf("%s/charts/Adenovirus41_genomemap_mutations3.pdf", 
                            find_rstudio_root_file()), 
       width = 12, height = 5)
```
...
```{r}
length(unique(ordered_date_mut$sample_ID))
```
